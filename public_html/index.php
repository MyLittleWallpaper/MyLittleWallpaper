		...
		if (preg_match('/^\/c\/([0-9a-z_-]*)\/$/', $requestUri)) {
			...
			if (strcmp($uriParts[1 + $offset], 'errors') === 0) {
				...
				if (!empty($uriParts[2 + $offset]) && !empty($uriParts[3 + $offset])) {
					if (preg_match('/^[0-9a-zA-Z]*\.[0-9a-zA-Z]*$/', $uriParts[3 + $offset])
						&& file_exists(ROOT_DIR . 'pages/api/' . $uriParts[2 + $offset] . '/calls/' . Format::fileWithoutExtension($uriParts[3 + $offset]) . '.inc.php')
						&& file_exists(ROOT_DIR . 'pages/api/' . $uriParts[2 + $offset] . '/output/' . Format::fileExtension($uriParts[3 + $offset]) . '.inc.php')
					) {
				...
				$pageType = 'errors';
				$page = '404';
				if (!empty($uriParts[2 + $offset]) && preg_match('/^[0-9a-z]{14}\.[0-9a-z]{8}$/', $uriParts[2 + $offset])) {
					$pageType = 'link';
					$page = $uriParts[2 + $offset];
				}
				...
				$pageType = 'errors';
				$page = '404';
				if (!empty($uriParts[2 + $offset]) && strcmp($uriParts[2 + $offset], 'queue-image') === 0 && !empty($uriParts[3 + $offset])) {
					$image = substr($uriParts[3 + $offset], 0, 23);
					$page = 'queue-image';
					$pageType = 'moderate';
				} elseif (!empty($uriParts[2 + $offset]) && file_exists(ROOT_DIR . 'pages/moderate/' . $uriParts[2 + $offset] . '.php')) {
					$pageType = 'moderate';
					$page = $uriParts[2 + $offset];
				}
			} else {
...
if (strcmp($pageType, 'images') === 0) {
	...
	if (empty($pageType) || strcmp($pageType, 'feed') === 0 || strcmp($pageType, 'link') === 0 || strcmp($pageType, 'download') === 0 || strcmp($pageType, 'api') === 0) {
		// Redirect logic
		$redirectOk = true;
	}
	require_once(ROOT_DIR . 'inc/init.php');
