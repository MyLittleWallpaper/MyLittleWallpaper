  ...
  vexDialogFactory = function($, vex) {
    ...
    dialog.buttonsToDOM = function(buttons) {
      ...
      $.each(buttons, function(index, button) {
        if (button.type == "link") {
          return $buttons.append($("<a href=\"" + button.href + "\" target=\"_blank\"></a>").text(button.text).addClass(button.className + ' vex-dialog-button ' + (index === 0 ? 'vex-first ' : '') + (index === buttons.length - 1 ? 'vex-last ' : '')).bind('click.vex', function(e) {
            if (button.click) {
              button.click($(this).parents("." + vex.baseClassNames.content), e); return false;
            }
		  }));
        } else {
