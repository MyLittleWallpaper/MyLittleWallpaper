...
if (!empty($_GET['id'])) {
	...
	while($image = $result->fetch(PDO::FETCH_ASSOC)) {
		...
		foreach($authors as $tag) {
			...
			$pageContents .= '							<a href="' . PUB_PATH_CAT . '?search=' . urlencode('author:' . $tag->getName()) . '">' . Format::htmlEntities($tag->getName()) . '</a>';
		}
		$pageContents .= "\n".'						</div>'."\n";
		$pageContents .= '					</div>'."\n";
		...
		foreach($tags as $tag) {
			...
			$pageContents .= '							<a href="' . PUB_PATH_CAT . '?search=' . urlencode($tag->getName()) . '"' . ($class != '' ? ' class="' . $class . '"' : '') . '>' . Format::htmlEntities($tag->getName()) . '</a>';
		}
		$pageContents .= "\n".'						</div>'."\n";
		$pageContents .= '					</div>'."\n";
		...
		foreach($platforms as $tag) {
			...
			$pageContents .= '							<a href="' . PUB_PATH_CAT . '?search=' . urlencode('platform:' . $tag->getName()) . '">' . Format::htmlEntities($tag->getName()) . '</a>';
		}
		$pageContents .= "\n".'						</div>'."\n";
		$pageContents .= '					</div>'."\n";
