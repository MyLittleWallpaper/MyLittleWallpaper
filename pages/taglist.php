...
while($tag = $result->fetch(PDO::FETCH_ASSOC)) {
	...
	$html .= '<div class="taglist_tag"><a title="Tag type: '.Format::htmlEntities(ucfirst($tag['type'])).'" class="tagtype_'.Format::htmlEntities($tag['type']).'" href="'.PUB_PATH_CAT.'?search='.urlencode($tag['name']).'">'.Format::htmlEntities($tag['name']).'</a></div>';
}
