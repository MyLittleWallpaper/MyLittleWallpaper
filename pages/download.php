        ...
        if (file_exists(ROOT_DIR . $_ENV['FILE_FOLDER'] . $file['file'])) {
            if ($file['direct_with_link'] == '1') {
                ...
                foreach ($tag_authors as $k => $tag_author) {
                    $artists .= '<a href="' . PUB_PATH_CAT . '?search=author%3A' . urlencode($tag_author->getName()) .
                        '" target="_blank">' . Format::htmlEntities($tag_author->getName()) . '</a>';
                ...
                $downloadPage->setPageTitleAddition(
                    Format::htmlEntities($wallpaper->getName()) . ' by ' . $artists_header
                );
                ...
                $html    .= '			<div class="dldsize">' . $wallpaper->getWidth() . 'x' .
                    $wallpaper->getHeight() .
                    (!empty($aspects) ? ' (' . Format::htmlEntities($aspects[0]->getName()) . ')' : '') . '</div>' .
                    "\n";
                $html    .= '				<div class="dldbuttons">' . "\n";
                $html    .= '					<a class="download button" href="' .
                    Format::htmlEntities($wallpaper->getImageLink()) . '?download=1">Download</a>';
                if ($wallpaper->getUrl() !== '') {
                    $html .= '					<a class="button" style="margin-left:2px;" href="' .
                        $wallpaper->getUrl() . '" target="_blank">Visit author\'s website</a>';
                }
                ...
                foreach ($tags as $tag) {
                    ...
                    $html .= '				<a href="' . PUB_PATH_CAT . '?search=' . urlencode($tag->getName()) .
                        '"' . ($class != '' ? ' class="' . $class . '"' : '') . '>' .
                        Format::htmlEntities($tag->getName()) . '</a><br />' . "\n";
                }
                ...
                $html .= '				<span><label>Favs:</label> <span id="fav_count_' . $wallpaper->getId() .
                    '">' . $wallpaper->getFavourites() . '</span></span><br />';

                $html .= '			</div>' . "\n";
                $html .= '			<div class="dldimage">' . "\n";
                $html .= '				<div class="wallpaperdld">' . "\n";
                $html .= '					<img src="' . Format::htmlEntities($wallpaper->getImageLink()) .
                    '" alt="' . Format::htmlEntities($wallpaper->getName()) . ' by ' . $artists_header .
                    '" onclick="download_image_enlarge(this, ' . $wallpaper->getWidth() . ');" />' . "\n";
                $html .= '				</div>' . "\n";
                $html .= '			</div>' . "\n";
                ...
                $meta .= '		<meta name="twitter:image:src" content="' .
                    Format::htmlEntities($wallpaper->getImageThumbnailLink(2)) . '" />' . "\n";
            } else {
