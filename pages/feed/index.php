...
$output .= '<atom:link href="' . Format::xmlEntities(PROTOCOL . SITE_DOMAIN . $_SERVER['REQUEST_URI']) . '" rel="self"></atom:link>';
...
if (!empty($wallpapers)) {
	foreach ($wallpapers as $wallpaper) {
		...
		$output .= '<description>' . Format::xmlEntities('<img src="' . Format::xmlEntities($wallpaper->getImageThumbnailLink(2)) . '" alt="" />') . '</description>';
		$output .= '<pubDate>' . gmdate('Y-m-d', $wallpaper->getTimeAdded()) . 'T' . gmdate('H:i:s', $wallpaper->getTimeAdded()) . 'Z</pubDate>';
		...
		if (!empty($authors)) {
			foreach($authors as $author) {
				$output .= '<media:credit role="author" scheme="urn:ebu">'.Format::xmlEntities($author->getName()).'</media:credit>';
			}
		}
		$output .= '<media:content url="'.Format::xmlEntities($wallpaper->getImageLink()).'" height="'.$wallpaper->getHeight().'" width="'.$wallpaper->getWidth().'" medium="image" />';
		$output .= '</item>';
	}
}
