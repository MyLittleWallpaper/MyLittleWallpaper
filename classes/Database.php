...
class Database {
	 ...
	 * @throws PDOException
	 */
	public function checkUnique($value, $table, $field, $id = 0) {
		$query = " SELECT ";
					...
					if (is_array($condition['field'])) {
						...
						foreach ($condition['field'] as $fld) {
							...
							if (!empty($condition['value'])) {
								...
								$conditionsString .= "`" . $condition['table'] . "`.`" . $fld . "` " . $operator . " NULL";
							}
						}
						$conditionsString .= ")";
					} else {
						if (!is_null($condition['value'])) {
							$conditionsString .= "`" . $condition['table'] . "`.`" . $condition['field'] . "` " . $operator . " ?";
							$assign_data[] = $condition['value'];
						} else {
							if (strcmp('!=', $operator) === 0) {
								$operator = 'IS NOT';
							...
							$conditionsString .= "`" . $condition['table'] . "`.`" . $condition['field'] . "` " . $operator . " NULL";
						}
					}
				}
			}
			...
			foreach ($conditions as $condition) {
				...
				if (!empty($condition['operator']) && strcasecmp('in', $condition['operator']) === 0) {
					...
					if (is_array($condition['field'])) {
						...
						if (!is_null($condition['value'])) {
							$condition_str .= "`" . $condition['table'] . "`.`" . $condition['field'] . "` " . $operator . " ?";
							$assign_data[] = $condition['value'];
						} else {
							if (strcmp('!=', $operator) === 0) {
								$operator = 'IS NOT';
							...
							$condition_str .= "`" . $condition['table'] . "`.`" . $condition['field'] . "` " . $operator . " NULL";
						}
					}
				}
			}
	...
	public function getRecord($table, $id, $fields = [], $join = []) {
		...
		if (!empty($join)) {
			foreach ($join as $j) {
				$select .= "JOIN `" . $j['table'] . "` ON (`" . $j['condition'][0]['table'] . "`.`" . $j['condition'][0]['field'] . "` = `" . $j['condition'][1]['table'] . "`.`" . $j['condition'][1]['field'] . "`) ";
			}
		}
		if (!is_array($id)) {
	 ...
	 * @throws PDOException
	 */
	public function saveArray($table, $data, $id = 0) {
		if (!empty($table)) {
			if (!empty($data) && is_array($data)) {
				if (empty($id)) { // Saves a new record
					...
					$query = "INSERT ";
					$query .= "INTO `" . str_replace('`', '', $table) . "` (" . $insert_fields . ") VALUES (" . $insert_data . ")";
					...
					$this->query("UPDATE `" . str_replace('`', '', $table) . "` SET " . $update_data . " WHERE id = ?", $update_data_array);
