...
class GetCommonColours
{
     ...
     * @var int
     */
    private int $PREVIEW_WIDTH = 768;
    /**
     * @var int
     */
    private int $PREVIEW_HEIGHT = 768;
    ...
    public function getColours(string $img): array
    {
        ...
        exec(
            ...
            "\> -dither None -remap " . ROOT_DIR . "classes/palette.gif " . $tempName
        );
        ...
        if (!empty($im)) {
            ...
            foreach ($colourList as $colour => $amount) {
                if (isset($this->colours[$colour]) && isset($groups[$this->colours[$colour]])) {
                    $groups[$this->colours[$colour]]['colours'][$colour] = $amount;
                }
            }
            foreach ($groups as $gk => $gv) {
                natcasesort($gv['colours']);
