...
class GetCommonColors {
	...
	function get_colors($img) {
		$tempnam = uniqid().'.gif';
		exec("convert ".escapeshellarg($img)." -resize ".$this->PREVIEW_WIDTH."x".$this->PREVIEW_HEIGHT."\> -dither None -remap ../palette.gif ../temp/".$tempnam);
		...
		if ($im) {
			...
			for ($y=0; $y < $imgHeight; $y++) {
				for ($x=0; $x < $imgWidth; $x++) {
					...
					if (!isset($colourlist[$hexcode])) $colourlist[$hexcode] = 1; else $colourlist[$hexcode] ++;
					if (isset($this->colours[$hexcode])) if (!isset($groupstemp[$this->colours[$hexcode]])) $groupstemp[$this->colours[$hexcode]] = 1; else $groupstemp[$this->colours[$hexcode]] ++;
				}
			}
			imagedestroy($im);
...
if (PHP_SAPI == 'cli') {
	...
	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
		...
		foreach($result as $cl) {
			...
			$savedata = Array('wallpaper_id' => $row['id'], 'tag_r' => $tag_r, 'tag_g' => $tag_g, 'tag_b' => $tag_b, 'tag_colour' => $col, 'amount' => round($amnt, 2));
