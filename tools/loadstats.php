...
if (PHP_SAPI == 'cli') {
	...
	$savedata = Array('time' => gmdate('Y-m-d H:i:s'), 'avg1' => $loadavg[0], 'avg5' => $loadavg[1], 'avg15' => $loadavg[2], 'users_online' => $usersonline);
	...
	if ($beforestamp == gmmktime(gmdate('H'), 0, 0, gmdate('n'), gmdate('j'), gmdate('Y')) || $beforestamp == gmmktime(gmdate('H'), 30, 0, gmdate('n'), gmdate('j'), gmdate('Y'))) {
		$sql = "SELECT AVG(load_time) load_time, MAX(load_time) damax FROM page_loadtime WHERE `time` < ? AND `time` >= ?";
		$res = $db->query($sql, Array($before, gmdate('Y-m-d H:i:s', $beforestamp - 1800)));
	        $loadtime = 0;
		$loadtimemax = 0;
        	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
		...
		$savedata = Array('time' => $before, 'load_time' => round($loadtime, 4), 'load_time_max' => round($loadtimemax, 4));
	...
	if ($beforestamp == gmmktime(gmdate('H'), 0, 0, gmdate('n'), gmdate('j'), gmdate('Y'))) {
		...
		$res = $db->query($sql, Array($before, gmdate('Y-m-d H:i:s', $beforestamp - 3600)));
		$pageviews = 0;
		while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
			$pageviews = $row['cnt'];
		}
		$savedata = Array('time' => $before, 'views' => $pageviews);
