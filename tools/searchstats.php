...
$res = $db->query(
    "SELECT tag, count(*) searches FROM `tag_searchstats` 
    WHERE type = 'tag' GROUP BY tag ORDER BY searches DESC LIMIT 10"
);
while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
...
$res = $db->query(
    "SELECT tag, count(*) searches FROM `tag_searchstats`
    WHERE type = 'tag' AND time >= ? AND time < ? GROUP BY tag ORDER BY searches DESC LIMIT 10",
    [gmdate('Y-m-d H:i:s', $after24stamp), gmdate('Y-m-d H:i:s', $beforestamp)]
);
while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
    $percent = round($row['searches'] / $total * 100, 2);
...
$res = $db->query(
    "SELECT tag, count(*) searches FROM `tag_searchstats`
    WHERE type = 'author' GROUP BY tag ORDER BY searches DESC LIMIT 10"
);
while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
    $percent = round($row['searches'] / $total * 100, 2);
    $str     .= '<tr>';
...
$res = $db->query(
    "SELECT tag, count(*) searches FROM `tag_searchstats`
    WHERE type = 'author' AND time >= ? AND time < ? GROUP BY tag ORDER BY searches DESC LIMIT 10",
    [gmdate('Y-m-d H:i:s', $after24stamp), gmdate('Y-m-d H:i:s', $beforestamp)]
);
while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
    $percent = round($row['searches'] / $total * 100, 2);
