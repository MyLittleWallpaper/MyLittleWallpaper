...
if (PHP_SAPI == 'cli') {
	...
	$res = $db->query("SELECT url, time FROM visit_log WHERE time < ? AND time >= ? AND id > 171428 ORDER BY time", [gmdate('Y-m-d H:i:s', $beforestamp), gmdate('Y-m-d H:i:s', $afterstamp)]);
	$cnt = 0;
			...
			if ($searchstring != '') {
				...
				foreach($searchparts as $searchpart) {
					if (substr($searchpart, 0, 9) == 'platform:') {
						...
						$result = $db->query("SELECT id, name FROM tag_platform WHERE name = ?", [substr($searchpart, 9)]);
	...
	$res = $db->query("SELECT count(*) searches FROM `tag_searchstats` WHERE type = 'tag' AND time >= ? AND time < ?", [gmdate('Y-m-d H:i:s', $after24stamp), gmdate('Y-m-d H:i:s', $beforestamp)]);
	$total = 0;
	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
		$total = $row['searches'];
	}

	$res = $db->query("SELECT tag, count(*) searches FROM `tag_searchstats` WHERE type = 'tag' AND time >= ? AND time < ? GROUP BY tag ORDER BY searches DESC LIMIT 10", [gmdate('Y-m-d H:i:s', $after24stamp), gmdate('Y-m-d H:i:s', $beforestamp)]);
	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
		$percent = round($row['searches'] / $total * 100, 2);
		$str .= '<tr>';
		$str .= '<td style="width:200px;border-bottom:1px solid #ccc;padding:4px 0;font-weight:bold;">'.Format::htmlEntities($row['tag']).'</td>';
	...
	$after24stamp = $beforestamp - (3600 * 24);
	$res = $db->query("SELECT count(*) searches FROM `tag_searchstats` WHERE type = 'author' AND time >= ? AND time < ?", [gmdate('Y-m-d H:i:s', $after24stamp), gmdate('Y-m-d H:i:s', $beforestamp)]);
	$total = 0;
	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
		$total = $row['searches'];
	}
	
	$res = $db->query("SELECT tag, count(*) searches FROM `tag_searchstats` WHERE type = 'author' AND time >= ? AND time < ? GROUP BY tag ORDER BY searches DESC LIMIT 10", [gmdate('Y-m-d H:i:s', $after24stamp), gmdate('Y-m-d H:i:s', $beforestamp)]);
	while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
		$percent = round($row['searches'] / $total * 100, 2);
		$str .= '<tr>';
		$str .= '<td style="width:200px;border-bottom:1px solid #ccc;padding:4px 0;font-weight:bold;">'.Format::htmlEntities(substr($row['tag'], 7)).'</td>';
